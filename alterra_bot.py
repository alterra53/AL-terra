...
class VerifyButton(nextcord.ui.View):
    ...
    @nextcord.ui.button(
        ...
        emoji="✔️",
        custom_id="alterra_verify_button"
    )
    async def verify(self, button, interaction: Interaction):
        ...
        if guild_id not in config or "verify_role" not in config[guild_id]:
            await interaction.response.send_message(
        ...
        role_id = config[guild_id]["verify_role"]
        role = guild.get_role(role_id)

        if not role:
            await interaction.response.send_message(
                "Verification role is invalid.",
                ephemeral=True
            )
            return
